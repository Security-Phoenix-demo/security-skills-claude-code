...
Threat intelligence search across 595 curated security domains in 4 authority tiers — CVEs,
threat actors, malware families, exploits, breach reports.

| Component | What it is | Needs |
|---|---|---|
| `cti-domain-research` skill | Tiered search using Claude's own web-search tools | Nothing |
| `/cti-search` command | Wrapper over the bundled Node CLI | Node 18+, a search API key |
| `index.js` | Standalone CLI | Node 18+, a search API key |
| `mcp-server.js` | MCP tool server (register it yourself) | Node 18+, a search API key |

Start with the skill. It needs no setup at all. Move to the CLI when you want speed,
scripting, or the NotebookLM push.
...
Then, only if you want the CLI or the MCP server:

```bash
MP=~/.claude/plugins/marketplaces/phoenix-security
cd "$MP/plugins/phoenix-cti-search"
...
> A `/plugin update` replaces this directory. Copy your `.env` somewhere safe first.

...
Two skills for generating opengrep/semgrep SAST rules.

| Skill | Use when |
|---|---|
| `opengrep-rule-generator` | You know the pattern you want to detect, or you have a CVE/CWE reference to hand |
| `opengrep-rule-generator-research` | You need to research the vulnerability with web search first, then write rules from the findings |
...
Neither skill needs an API key or a runtime. You need `opengrep` or `semgrep` installed only
to *run* the rules they produce.

Full options: [MARKETPLACE_INSTALL.md](../../MARKETPLACE_INSTALL.md).
...
│   ├── SKILL.md
│   ├── RULES_SYNTAX.md                # the full rule-syntax reference
│   └── OPENGREP_RULE_GENERATOR_PROMPT.md

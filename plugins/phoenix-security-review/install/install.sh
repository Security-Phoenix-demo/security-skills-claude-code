#!/usr/bin/env bash
# install.sh — wires the phoenix-security-review hooks, commands and subagent into a project.
#
# You do NOT need this to use the skills. Installing the plugin is enough for the six skills,
# the four slash commands and the subagent. This script exists to wire the ACTIVE HOOKS,
# which are deliberately opt-in, and to support Windsurf and Codex.
#
# Usage (run from your project root):
#   PLUGIN=~/.claude/plugins/marketplaces/phoenix-security/plugins/phoenix-security-review
#   bash "$PLUGIN/install/install.sh"              # lite: the SessionEnd reminder hook only
#   bash "$PLUGIN/install/install.sh" --full       # full: all 4 hooks
#   bash "$PLUGIN/install/install.sh" --standalone # also copy the commands + subagent into
#                                                  # .claude/ — for projects NOT using the plugin
#   bash "$PLUGIN/install/install.sh" --uninstall
#   bash "$PLUGIN/install/install.sh" --dry-run    # show what would change
#
# What it does (idempotent):
#   - Merges the chosen hook preset into .claude/settings.json (creates if missing), with
#     absolute paths resolved to this plugin's real location
#   - chmods hook scripts so SessionStart / PreToolUse / PostToolUse can run
#   - --standalone only: copies the 4 slash commands into .claude/commands/ and the
#     security-reviewer subagent into .claude/agents/. Skip this when the plugin is
#     installed — it already provides both, and duplicate slash names collide.
...
if [[ "$UNINSTALL" == 1 ]]; then
  say "uninstalling…"
  for f in security-0day security-review security-audit security-assessment threatmodel; do
...
# ---- 1. slash commands + subagent (--standalone only) -----------------------
# The plugin already provides both. Copying them into the project as well creates duplicate
# slash names. Only do it when the project is not using the plugin.
if [[ "$STANDALONE" == 1 ]]; then
  do_or_show "mkdir -p \"$CMD_DIR\""
  for f in security-0day security-review security-audit threatmodel; do
    src="$SUITE_DIR/commands/$f.md"
    dst="$CMD_DIR/$f.md"
    [[ -f "$src" ]] || { say "! command source missing: $src"; continue; }
    if [[ -f "$dst" ]] && cmp -s "$src" "$dst"; then continue; fi
    do_or_show "cp \"$src\" \"$dst\""
  done
  say "✓ slash commands copied: /security-0day, /security-review, /security-audit, /threatmodel"

  do_or_show "mkdir -p \"$AGENT_DIR\""
  src="$SUITE_DIR/agents/security-reviewer.md"
  dst="$AGENT_DIR/security-reviewer.md"
  if [[ -f "$src" ]]; then
    do_or_show "cp \"$src\" \"$dst\""
    say "✓ subagent copied: $dst"
  else
    say "! subagent source missing at $src — is the plugin directory intact?"
  fi
else
  say "· skipping commands + subagent (the plugin provides them; pass --standalone to copy anyway)"
fi

# ---- 3. chmod hook scripts --------------------------------------------------
do_or_show "chmod +x \"$SUITE_DIR/install/hooks/session-end-security-0day.sh\""
if [[ "$PRESET" == "full" ]]; then
  do_or_show "chmod +x \"$SUITE_DIR/hooks/\"*.sh 2>/dev/null || true"
fi
say "✓ hook scripts executable"

# ---- 4. settings.json merge -------------------------------------------------
preset_src="$SUITE_DIR/install/hooks/settings.$PRESET.example.json"
[[ -f "$preset_src" ]] || { say "! preset file missing: $preset_src"; exit 1; }

# The preset carries __SUITE_DIR__ placeholders. Resolve them to this plugin's real absolute
# path, so the hooks work wherever the plugin was installed (marketplace cache, local
# checkout, or a copy inside the project).
preset_file="$(mktemp)"
trap 'rm -f "$preset_file"' EXIT
sed "s|__SUITE_DIR__|$SUITE_DIR|g" "$preset_src" > "$preset_file"
...
if [[ "$STANDALONE" == 1 ]]; then
  ...
  say "  - The commands come from the plugin. Type / and verify they appear."
fi
if [[ "$PRESET" == "full" ]]; then

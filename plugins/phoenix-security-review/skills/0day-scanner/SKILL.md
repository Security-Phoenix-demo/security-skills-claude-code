...
name: 0day-scanner
description: >
  LLM-powered zero-day vulnerability analysis of code changes — a commit, a pull request,
  a file or a branch — in light, standard or deep mode, with cost budgeting and
  knowledge-graph linking. Use whenever someone asks to scan for vulnerabilities, check
  for security issues, mentions 0-day or zero-day, or wants a specific commit, PR, diff
  or branch reviewed for security. Prefers the `analyze_for_zero_day_vulnerabilities` MCP
  tool and falls back to the per-language reference packs plus the 8-point check scoped to
  the diff. For a whole-repo pass use `security-assessment`.
---

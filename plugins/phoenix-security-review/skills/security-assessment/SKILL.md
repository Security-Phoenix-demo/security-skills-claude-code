...
description: >
  Comprehensive OWASP Top 10 2025 and ASVS Level 1 security assessment across a whole
  codebase, with cost budgeting, knowledge-graph linking and automated test generation.
  Use whenever someone asks to run a security assessment, a security audit, a security
  check or a vulnerability assessment; mentions OWASP Top 10 or ASVS; or wants a full
  security pass before a release. Prefers the `run_security_assessment` MCP tool and falls
  back to the bundled OWASP/ASVS checklists and per-language reference packs when MCP is
  unreachable. For a diff-scoped pass use `0day-scanner`; for design risk use
  `threat-modeling`; for a threat-model-driven review use `tm-security-review`.
---

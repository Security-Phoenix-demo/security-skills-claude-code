...
- Phase 0 → **`threat-modeling`** (`../Security Assessment/threat-modeling/SKILL.md`) — STRIDE/DREAD, DFD, trust boundaries.
- Phases 1-3 → **`0day-scanner`** (`../Security Assessment/0day-scanner/SKILL.md`) — the LLM exploit engine, run in `deep` mode, steered by the model's KEY threats. Its on-disk fallback (`security-reviewer/languages/*` + 8-point check) applies when the MCP tool is unreachable.
